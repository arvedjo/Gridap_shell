...
function solve()
    ...
    section = begin_timed_section!(to, "Setting up model parameters and geometry")
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # == Section 2: FE Spaces ==
    section = begin_timed_section!(to, "Creating FE spaces")
    ...
    section = begin_timed_section!(to, "Setting up kinematics and geometry functions")
    ...
    """
    function normal_vector(T::Tuple{VectorValue{3,Tv},VectorValue{3,Tv}}) where {Tv<:Number}
    ...
    section = begin_timed_section!(to, "Defining strain and stress tensors")
    ...
    """
    function κ(∇u, β, ∇β)
        return -0.5 * (F(∇u)' ⋅ ∇d(β + β₀, ∇β + ∇β₀)' + ∇d(β + β₀, ∇β + ∇β₀) ⋅ F(∇u)) - b₀
    ...
    section = begin_timed_section!(to, "Setting up weak form")
    ...
    # )
    wh = interpolate([VectorValue(0.0, 0.0, 0.0), VectorValue(0.0, 0.0)], X)
    ...
    #solve
    section = begin_timed_section!(to, "Starting nonlinear solver")
    ...
    for step in 0:20
        load_z = step * -100.0
        wh, cache = run(wh, load_z, step, 21, cache)
    end

    return end_timed_section!(to, section)
end
